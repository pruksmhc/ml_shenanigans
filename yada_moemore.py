...
class MLPFFN(nn.Module):
    def __init__(self, config):
        ...
        # Define the layers inside a ModuleDict
        self.mlp = nn.ModuleDict({
            'c_fc': nn.Linear(config.n_embd, 4 * config.n_embd),
            'c_proj': nn.Linear(4 * config.n_embd, config.n_embd),
            'act': NewGELU(),  # Assuming NewGELU() is already defined elsewhere
        })

    def forward(self, x):
        # Apply the layers step by step
        x = self.mlp['c_fc'](x)  # First linear layer
        x = self.mlp['act'](x)   # Activation function (GELU)
        x = self.mlp['c_proj'](x)  # Second linear layer (projection)
        return x
...
class SparseMOE(nn.Module):
  ...
  def forward(self, x):
        ...
        updates = torch.zeros_like(flat_x) #[batch_size * seq_len, n_embd]
        ...
        for i, expert in enumerate(self.experts):
            ...
            if limited_indices.numel() > 0:
                ...
                expert_output = expert(expert_input) # size: [indices_with_expert_i_selected, n_embed]
...
class Transformer(nn.Module):
    """ Transformer Language Model, ex actly as seen in GPT-2 """
